<?php
    ...
    function toDataBase($watermark, $fileExt, $fileName, $title, $author, $private){
        $db = get_db();
        ...
        if($private === 'private'){
            $db->galleryPrivate->insertOne($image);
        }
        else{
    ...
    function getImages(){
        $db = get_db();
        $images = $db->gallery->find()->toArray();
        return $images;
    }

    function getImagesPrivate($author){
        $db = get_db();
        $temp = ['author' => $author];
        $images = $db->galleryPrivate->find($temp)->toArray();
        return $images;
    }
